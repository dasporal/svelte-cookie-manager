...
{#if shouldRender && !isManaging}
	...
	{:else if displayType === 'banner'}
		<div
			class={`fixed bottom-4 left-1/2 w-full -translate-x-1/2 md:max-w-2xl ${theme === 'light' ? 'border border-black/10 bg-white/95 shadow-lg' : 'bg-black/90 text-white ring-1 ring-white/10'} 
	...
	{:else if displayType === 'popup'}
		<div
			class={`fixed bottom-4 left-4 w-80
             ${theme === 'light' ? 'bg-white/95 shadow-lg ring-1 ring-black/10' : 'bg-black/90 text-white ring-1 ring-white/10'}
	...
	{:else}
		<div
			class={`fixed inset-0 flex items-center justify-center p-4
             ${theme === 'light' ? 'bg-black/20 backdrop-blur-sm' : 'bg-black/90 text-white backdrop-blur-sm'}
